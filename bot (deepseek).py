...
from typing import Any, Optional, List, Dict, Set, Tuple, Callable, Coroutine
...
# Admin commands: restrict to allowed users (could be extended with a separate admin list)
ADMIN_IDS = ALLOWED_USER_IDS  # for now, same as allowed users
...
# ----------------------------------------------------------------------
# Database initialization (with new index)
# ----------------------------------------------------------------------
...
async def access_control(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user is None or user.id in ALLOWED_USER_IDS:
        return
    logger.warning("Blocked from unauthorized user_id=%s", user.id)
    now = time.monotonic()
...
# ----------------------------------------------------------------------
# Helper functions for parsing and batch summarisation (unchanged)
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# Command handlers (collect, fav, current, finish, stop, minlength, removemode)
# ----------------------------------------------------------------------
...
async def handle_video(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Save concurrently to all collections
...
# Tokens for list pagination
_list_pages: Dict[str, Tuple[List[Tuple[str, int]], int, str]] = {}  # token -> (rows, page, search)
...
async def list_collections(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Build inline buttons for pagination
    token = _list_token(chat_id, search or "")
    _list_pages[token] = (rows, page, search or "")

    buttons = []
    # Page jump buttons (only if more than one page)
    if total_pages > 1:
        nav_row = []
        if page > 1:
    ...
    # Set/Get buttons for each collection
...
# Existing list choice/set/get callbacks (unchanged)
...
async def _send_album_group(
    ...
    """Try to send as media group. Returns (messages, failed_flag)."""
...
async def _send_album_individually(
    ...
    """Send each video individually. Returns (sent, failed, sent_records)."""
    sent = 0
    failed = 0
    records = []
...
async def _send_page(
    ...
    """Send albums with progress editing. Returns (sent_records, album_statuses)."""
...
async def _do_send_page(
    ...
    if next_offset >= len(rows):
        ...
        # Offer auto-cleanup if dead files were detected
        ...
        if dead_count > 0:
            ...
            await update.effective_message.reply_text(
                f"ℹ️ {dead_count} dead file(s) were detected during sending. Run cleanup?",
                reply_markup=keyboard,
            )
    else:
...
# ----------------------------------------------------------------------
# /rename, /merge, /copy, /export, /delete (unchanged, but admin check added)
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# /status, /random, /neardupes, /dups, /recent, /cleanup, /stats
# ----------------------------------------------------------------------
...
async def random_video(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if args:
        ...
        if len(active) == 1:
            ...
            # Show choice
            buttons = [[InlineKeyboardButton(c, callback_data=f"randchoice:{c}")] for c in active]
            await update.message.reply_text("Which collection?", reply_markup=InlineKeyboardMarkup(buttons))
            return

    await _send_random(update, context, name, count)

async def random_choice_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    name = query.data[len("randchoice:"):]
    await query.answer()
    await query.edit_message_text(f"🎲 Random from '{name}'...")
    await _send_random(update, context, name, 1)

async def _send_random(update: Update, context: ContextTypes.DEFAULT_TYPE, name: str, count: int):
    chat_id = update.effective_chat.id
    try:
        def _query(conn):
            with conn.cursor() as cur:
        ...
        await reply_db_error(update, "random")
        return
    if not rows:
        await update.message.reply_text(f"No videos in '{name}'.")
        return
    ...
    for idx, (fid, fuid, dur, size, added) in enumerate(selected):
        ...
        try:
            ...
            await update.message.reply_text("⚠️ Failed to send (dead file). Try again.")
...
async def find_videos(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Parse: first argument is collection, rest are filters
    ...
    if size_filter:
        try:
            ...
            params.append(val * 1024 * 1024)  # convert to bytes
        except ValueError:
            pass
...
async def post_init(application: Application):
    ...
    await application.bot.set_my_commands([
        ...
        BotCommand("list", "List collections"),
        BotCommand("get", "Send collection videos"),
        ...
        BotCommand("random", "Send random video(s)"),
...
async def run():
    ...
    application.add_handler(CallbackQueryHandler(random_choice_callback, pattern=r"^randchoice:"))
