...
async def _send_page(
    ...
    for album_idx, i in enumerate(range(0, len(page_rows), ALBUM_SIZE)):
        ...
        # 3s between albums keeps us well under Telegram's 20 messages/min flood limit.
        # This is also where /stop cancellation takes effect between albums.
...
async def _get_collection_impl(
    ...
    if next_offset >= total:
        await update.message.reply_text(f"✅ Done — all {total} video(s) from '{name}' sent.")
    else:
