...
async def list_collections(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = build_collection_select_keyboard(page_rows)
    await update.message.reply_text(
        f"{header}:\n" + "\n".join(lines) + footer,
...
def build_collection_select_keyboard(rows: list) -> InlineKeyboardMarkup | None:
    """One button per collection so the user can tap to make it the active
    collection instead of typing /collect <name>. Skips any name that would
    make callback_data exceed Telegram's 64-byte limit (rare in practice)."""
    buttons = []
...
async def set_collection_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handles taps on the per-collection buttons shown under /list, setting
    that collection as the sole active one — equivalent to /collect <name>."""
    query = update.callback_query
    _, name = query.data.split(":", 1)
    chat_id = update.effective_chat.id

    if name in RESERVED_NAMES:
        await query.answer(f"⚠️ '{name}' is reserved and can't be used as a collection.", show_alert=True)
        return

    active_collections[chat_id] = [name]
    paused_chats.discard(chat_id)  # tapping a collection resumes saving if /stop paused it
    await query.answer(f"📁 Active collection set to: {name}")
...
def main():
    ...
    application.add_handler(CallbackQueryHandler(set_collection_callback, pattern=r"^setcol:"))
